...
type FileStatus = "queued" | "processing" | "completed" | "failed" | "invalid";
...
export default function DocumentUploadZone() {
        ...
        setEntries((prev) => {
          const updated = prev.map((e) =>
            e.contentId && updates[e.contentId]
              ? { ...e, status: updates[e.contentId] as FileStatus }
              : e
          );
  ...
  const processedCount = entries.filter((e) => e.status === "completed" || e.status === "failed").length;
  ...
  return (
    <>
      {isPageDragOver && !isActive && (
        <div
          onDrop={(e) => { e.preventDefault(); setIsPageDragOver(false); dragCounterRef.current = 0; addFiles(e.dataTransfer.files); }}
          onDragOver={(e) => e.preventDefault()}
          className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background/90 backdrop-blur-sm border-4 border-dashed border-primary m-4 rounded-2xl pointer-events-auto"
        >
          ...
          <p className="text-sm text-muted-foreground mt-1">PDF or DOCX, max 25MB each</p>
        </div>
      )}
      ...
      {showDropZone && (
        <div
          ...
          className={cn(
            ...
            entries.length > 0 ? "py-8 px-6" : "py-20 px-6"
          )}
        >
          <input
            ref={fileInputRef}
          ...
          />
          <div className={cn(
            "mx-auto rounded-full flex items-center justify-center mb-4 transition-colors",
            isDragOver ? "bg-primary/10" : "bg-muted",
            entries.length > 0 ? "w-12 h-12" : "w-16 h-16"
          )}>
            <svg
              className={cn(
                "text-primary transition-all",
                isDragOver ? "scale-110" : "",
                entries.length > 0 ? "w-6 h-6" : "w-8 h-8"
              )}
          ...
          {entries.length === 0 ? (
            <>
              <p className="text-base font-medium text-foreground mb-1">
                {isDragOver ? "Release to add files" : "Drop your files here"}
              </p>
              <p className="text-sm text-muted-foreground mb-4">
                or <span className="text-primary font-medium">browse from your computer</span>
              </p>
              <p className="text-xs text-muted-foreground">PDF or DOCX — max 25MB per file, up to {MAX_FILES} files</p>
            </>
          ) : (
            <>
              <p className="text-sm text-muted-foreground">
                Drop more files or <span className="text-primary font-medium">browse</span>
              </p>
